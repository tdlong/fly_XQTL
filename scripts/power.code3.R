...
set.seed(myseed)
library(tidyverse)
libby = read.table("libby/libby2.txt")
...
MM = matrix(nrow=2*2*3*length(ALLPOS)*1,ncol=9)
cc=1
for(intensity in c(0.05,0.10)){
	for(NNind in c(300,600)){
	for(NgenRandomMate in c(4)){
...
fileout=paste("powerdir3/powersim.",myseed,".txt",sep="")
